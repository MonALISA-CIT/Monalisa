...
public class AFOXFullUpdateRequestMsg implements IHeader, Serializable {

	private static final long serialVersionUID = -5664009438811519138L;
	...
	private AFOXFullUpdateRequestMsg() { }

	public AFOXFullUpdateRequestMsg(
			String UserName_in,
	...
	public static AFOXFullUpdateRequestMsg DeSerialize(ByteArrayOutputStream ms) {
		AFOXFullUpdateRequestMsg ci = FromFlatSer(ms.toByteArray());
		return ci;
	}

	public static AFOXFullUpdateRequestMsg DeSerialize(byte[] b) {
		AFOXFullUpdateRequestMsg ci = FromFlatSer(b);
		return ci;
	}
	...
	public static AFOXFullUpdateRequestMsg FromFlatSer(byte[] ba) {
		UnflatSerialize uf = new UnflatSerialize(ba);
		return(FromFlatSer(uf));
	} 

	public static AFOXFullUpdateRequestMsg FromFlatSer(UnflatSerialize uf) {
		AFOXFullUpdateRequestMsg ci = null;  // ci = class instance.  init to null in case of error later so we have our, (defalut-error), return value.
		...
		while(true) { //while loop only done one time.  a "trick" for quick branching out if any step fails.
			...
			{ ci = new AFOXFullUpdateRequestMsg(
					UserName_in,
					MyCurSMUpdateID_in,
	...
	public int getHeader() {
		return AFOXMsgs.TCP_AFOXFullUpdateRequestMsg; 
	}
}
