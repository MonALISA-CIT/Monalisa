...
public class AFOXSinglePendingStatusRequestMsg implements Serializable, IHeader {

	private static final long serialVersionUID = -4804728185913698738L;

	public int Header = getHeader();  
	public String UserName;
	public int PendingID;

	private AFOXSinglePendingStatusRequestMsg() { }

	public AFOXSinglePendingStatusRequestMsg(
			String UserName_in,
			int PendingID_in) {
		UserName = UserName_in;
		PendingID = PendingID_in;
	}
	...
	public static AFOXSinglePendingStatusRequestMsg DeSerialize(ByteArrayOutputStream ms) {
		AFOXSinglePendingStatusRequestMsg ci = FromFlatSer(ms.toByteArray());
		return ci;
	}

	public static AFOXSinglePendingStatusRequestMsg DeSerialize(byte[] b) {
		AFOXSinglePendingStatusRequestMsg ci = FromFlatSer(b);
		return ci;
	}
	...
	public static AFOXSinglePendingStatusRequestMsg FromFlatSer(byte[] ba) {
		UnflatSerialize uf = new UnflatSerialize(ba);
		return(FromFlatSer(uf));
	} 

	public static AFOXSinglePendingStatusRequestMsg FromFlatSer(UnflatSerialize uf) {
		AFOXSinglePendingStatusRequestMsg ci = null;  // ci = class instance.  init to null in case of error later so we have our, (defalut-error), return value.
		...
		while(true) { //while loop only done one time.  a "trick" for quick branching out if any step fails.
			...
			{ ci = new AFOXSinglePendingStatusRequestMsg(
					UserName_in,
					PendingID_in
			);
			}
	...
	public int getHeader() {
		return AFOXMsgs.TCP_AFOXSinglePendingStatusRequestMsg; 
	}
}
