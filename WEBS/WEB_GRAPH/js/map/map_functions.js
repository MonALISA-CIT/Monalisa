	...
	manager = new GMarkerManager(map);
        ...
        GEvent.addListener(map, "singlerightclick", function(point,src,overlay){
    		if (overlay){
    			...
    			window.open(overlay.pointData["secondlink"], "window_"+overlay.pointData["label"]);
    		    }
    		}
    	    }
        );

        GDownloadUrl("map_data.jsp?relations=true&lines=true"+mapExtraOptions, function(data, responseCode){
            ...
            for (var i = 0; i < markers.length; i++) {
                ...
                var secondlink = markers[i].getAttribute("secondlink");
