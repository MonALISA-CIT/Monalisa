...
public class AppIperf implements lia.app.AppInt  {
	...
	"########### Required parameters : ############\n"
	+ "#parameters=parameters to be passed to iperf\n"
	+ "##############################################\n\n";
	...
	public boolean stop() {
		AppUtils.getOutput(new String[] { "/bin/killall", "iperf" });
	...
	public int status() {
		...
		String sRunning = AppUtils.getOutput(new String[]{"/bin/sh", "-c", "pstree -u `id -u -n`| grep iperf"});
	
		if (sRunning==null || sRunning.indexOf("iperf")<0)
	...
	public String exec(String sCmd) {
		return "exec has no meaning for the Iperf module";
	}
	...
	public boolean update(String vs[]) {
		try {
			...
			System.out.println(
					"iperf : exception : " + e + " (" + e.getMessage() + ")");
			e.printStackTrace();
			return false;
		}
	}
	...
	private void extractFiles() {
		try {
			...
			JarFile jf = new JarFile(MonaLisa_home + "/Control/lib/iperf.jar");
			...
			while (e.hasMoreElements()) {
				...
				if (s.endsWith("iperf")) {
					try {
						(new File(MonaLisa_home + "/Control/bin/iperf"))
						.delete();
					} catch (Exception ee) {
					}
					...
					new FileOutputStream(
							MonaLisa_home + "/Control/bin/iperf");
			...
			System.err.println("iperf: cannot extract: " + e + " (" + e.getMessage() + ")");
			e.printStackTrace();
		}
	}
