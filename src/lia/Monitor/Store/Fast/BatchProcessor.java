...
public final class BatchProcessor extends Thread {
    ...
    private static int DROP_ZEROES = AppConfig.geti("lia.Monitor.Store.Fast.BatchProcessor.DROP_ZEROES",
            BUFFER_SIZE / 10);
    ...
    private static final String sOverrunOutputFile = AppConfig.getProperty(
            "lia.Monitor.Store.Fast.BatchProcessor.OverrunLogFile", "overrun.sql");
     ...
     * @param mmax
     */
    public static void add(String tableName, long rectime, Result r, String paramName, double mval, double mmin,
            double mmax) {
    ...
    private static void addIDObject(final Vector<IDObject> vData, final String tableName, final int id,
            final long rectime, final double mval, final double mmin, final double mmax) {
        ...
        if (vData.size() > BUFFER_SIZE) {
            ...
            if ((System.currentTimeMillis() - lLastWarningLogged) > WARNING_PERIOD) {
                logger.log(Level.WARNING, "Buffer overrun, " + lDiscardedResults
                        + ", please do something to improve system's performance!\n Will try to log missing values to "
            ...
            synchronized (fileFlushLock) {
                try {
                    ...
                    pwDumper.println("INSERT INTO " + tableName + " VALUES (" + rectime + ","
                            + (id > 0 ? id + "," : "") + mval + "," + mmin + "," + mmax + ");");

                    bDumpedSomething = true;
                } catch (IOException e) {
                    logger.log(Level.WARNING, "Buffer overrun and I cannot write to " + sCannonicalOutputFileName
                            + " : " + tableName + " (" + rectime + "," + (id > 0 ? id + "," : "") + mval + "," + mmin
                            + "," + mmax + ")");
    ...
    private static boolean saveUpdateIDs(final ArrayList<MonitorIDs> al, final boolean lastseen) {
        ...
        for (int i = 0; i < al.size(); i++) {
            ...
            if (lastseen) {
                db.query("UPDATE monitor_ids SET mi_lastseen=" + mi.lastseen + ",mi_lastvalue=" + mi.lastvalue
                        + " WHERE mi_id=" + mi.id + ";");
            } else {
    ...
    private static int saveIDObjects() {
        ...
        for (int i = 0; i < vSize; i++) {
            ...
            q = "INSERT INTO " + o.sTable + " VALUES (" + o.rectime + "," + o.id + "," + o.mval + "," + o.mmin + ","
                    + o.mmax + ");";
            ...
            if (!db.query(q, true)) {
                if (hasDB()) {
                    ...
                    if (db.syncUpdateQuery("CREATE TABLE " + o.sTable
                            + " (rectime int, id int, mval real, mmin real, mmax real);", false)) {
                        db.syncUpdateQuery("CREATE INDEX " + o.sTable + "_idx ON " + o.sTable + " (id, rectime);",
                                false);
                        db.syncUpdateQuery("CREATE INDEX " + o.sTable + "_time_idx ON " + o.sTable + " (rectime);",
                                false);
    ...
    private static int saveIDObjects3() {
        ...
        for (int j = 0; j < alData.size(); j++) {
            ...
            final String q = "INSERT INTO " + o.sTable + " VALUES (" + o.rectime + ", " + o.mval + ", " + o.mmin + ", "
                    + o.mmax + ");";
    ...
    private static final void createV3Table(final String sTableName) {
        ...
        if (db.syncUpdateQuery("CREATE TABLE " + sTableName + " (rectime integer, mval " + DBWriter3.sFloatingPointDef
                + ", mmin " + DBWriter3.sFloatingPointDef + ", mmax " + DBWriter3.sFloatingPointDef + ") WITHOUT OIDS;")) {
        ...
        try {
            synchronized (sync) {
                ...
                if (no != vTimesTemp.size()) {
                    logger.log(Level.WARNING, " BatchProcessor different Results [ " + no + " ] and Times [ "
                            + vTimesTemp.size());
                    vTablesTemp.clear();
                    vParamsTemp.clear();
                    vValsTemp.clear();
                ...
                if (no != vTablesTemp.size()) {
                    logger.log(Level.WARNING, " BatchProcessor different Results [ " + no + " ] and Tables [ "
                            + vTablesTemp.size());
                    vParamsTemp.clear();
                    vValsTemp.clear();
                    vMinsTemp.clear();
                ...
                if (no != vParamsTemp.size()) {
                    logger.log(Level.WARNING, " BatchProcessor different Results [ " + no + " ] and Params [ "
                            + vParamsTemp.size());
                    vValsTemp.clear();
                    vMinsTemp.clear();
                    vMaxsTemp.clear();
                ...
                if (no != vValsTemp.size()) {
                    logger.log(Level.WARNING, " BatchProcessor different Results [ " + no + " ] and Vals [ "
                            + vValsTemp.size());
                    vMinsTemp.clear();
                    vMaxsTemp.clear();
                    return 0;
                ...
                if (no != vMinsTemp.size()) {
                    logger.log(Level.WARNING, " BatchProcessor different Results [ " + no + " ] and Mins [ "
                            + vMinsTemp.size());
                    vMaxsTemp.clear();
                    return 0;
                }
            ...
            if (no > 0) {
                ...
                for (int i = 0; i < no; i++) {
                    ...
                    String q = "INSERT INTO " + tableName
                            + " (rectime, mfarm, mcluster, mnode, mfunction, mval, mmin, mmax) VALUES (";
                    ...
                    q += lRectime.longValue() + "," + e(r.FarmName) + "," + e(r.ClusterName) + "," + e(r.NodeName)
                            + "," + e(paramName) + "," + dVal.doubleValue() + "," + dMin.doubleValue() + ","
                            + dMax.doubleValue() + ");";
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                if (count > 0) {
                    if (logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, "Saved " + count + " values (" + iStatements + ", " + iResults + ", "
                                + iIDObjects + ", " + iIDObjects3 + ") in " + (endTime - startTime) + " ms");
                    }
                ...
                if (++iRuns > 60) {
                    ...
                    synchronized (fileFlushLock) {
                        if (bDumpedSomething) {
                            ...
                            if ((pwDumper != null)
                                    && ((NTPDate.currentTimeMillis() - lLastDumpTime) > (1000 * 60 * 10))) {
                                pwDumper.close();
                                pwDumper = null;
                            }
                        }
