 ...
 * cluster
 */
public class MultipleBarClusterSummaryPlot implements LocalDataFarmClient, MultipleDataPlotter, ComponentListener,
        WindowListener, ActionListener {

    /** Logger used by this class */
    private static final Logger logger = Logger.getLogger(MultipleBarClusterSummaryPlot.class.getName());
    ...
    public MultipleBarClusterSummaryPlot(DataPlotterParent parent, rcNode[] selectedNodes, Vector predicates,
                ...
                for (String parameter : parameters) {
                    if (parameter.equals(rezParam)) {
                        if (!Double.isNaN(r.param[j]) && !Double.isInfinite(r.param[j])) {
                            if (multipleFarms) {
                                if ((currentUnit != null) && (baseUnit != null)
                                        && currentUnit.containsKey(r.param_name[j])) {
                                    Unit u = (Unit) currentUnit.get(r.param_name[j]);
                                    gcbc.addPoint(r.FarmName + "/" + r.NodeName, r.FarmName + "/" + rezParam, r.time,
                                            convert(u, r.param[j]));
                                } else {
                                    gcbc.addPoint(r.FarmName + "/" + r.NodeName, r.FarmName + "/" + rezParam, r.time,
                                            r.param[j]);
