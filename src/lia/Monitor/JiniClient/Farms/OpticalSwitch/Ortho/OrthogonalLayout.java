...
public class OrthogonalLayout {
	...
	public synchronized void update(int nrPorts, Link[] links) {
		readyToDraw = true;
		currentPozition = 0;
		xaxis = new TreeSet();
		yaxis = new TreeSet();
	...
	protected void initializeLinks(Link[] links) {
		
		if (links == null) return;
		orderredPorts = new TreeSet();
		for (int i=0; i<links.length; i++) {
			final Link link = links[i];
//			if (link.sourcePort().type() == PortType.MULTICAST_PORT || link.destinationPort().type == PortType.MULTICAST_PORT)
//				continue;
			if (!orderredPorts.contains(link.sourcePort().name()))
				orderredPorts.add(link.sourcePort().name());
			if (!orderredPorts.contains(link.destinationPort().name()))
				orderredPorts.add(link.destinationPort().name());
		}
...
//				continue;
			if (link.sourcePort().equals(link.destinationPort())) continue;
			if (v.contains(link.destinationPort()) || v.contains(link.sourcePort())) continue;
			v.add(link.sourcePort());
			v.add(link.destinationPort());
			Point2D.Double s = (Point2D.Double)namedPorts.get(link.sourcePort().name());
			Point2D.Double e = (Point2D.Double)namedPorts.get(link.destinationPort().name());
	...
	protected void initializeLinks(HashMap links) {
		...
		for (Iterator it = links.keySet().iterator(); it.hasNext(); ) {
			Link link = (Link)links.get(it.next());
			if (!orderredPorts.contains(link.sourcePort().name()))
				orderredPorts.add(link.sourcePort().name());
			if (!orderredPorts.contains(link.destinationPort().name()))
				orderredPorts.add(link.destinationPort().name());
