...
public class DataSelect {
	...
	public static final synchronized String rquery(monPredicate p, String tableName) {
		...
		if ((p.parameters != null) && (p.parameters.length > 0)) {
			...
			for (int i = 0; i < p.parameters.length; i++) {
				...
				if (p.parameters[i] == null || p.parameters[i].length() == 0)
					continue;
				...
				if (p.constraints != null && p.constraints[i] != null && p.constraints[i].length() > 0) {
					...
					for (int j = 0; j < AND_SplitConstraints.length; j++) {
						...
						if (j != AND_SplitConstraints.length - 1)
							rsb.append(" AND ");
					}
					rsb.append(" ) ");
				}
		...
		if (!bAnd)
			rsb.append(" true "); // in case there was no condition
	 ...
	 * @param r
	 * @param p
	 * @return the (partial) content that matches the predicate, if at all. <code>null</code> if nothing matches
	 */
	static final public Result matchResult(final Result r, final monPredicate p) {
		...
		if (!matchString(p.Farm, r.FarmName))
			return null;
		if (!matchString(p.Cluster, r.ClusterName))
			return null;
		if (!matchString(p.Node, r.NodeName))
			return null;
		...
		if (p.parameters != null) {
			...
			if (nr.param != null && nr.param.length > 0) {
				return nr;
			}
		}
		else {
	...
	private static final GenericLastValuesCache<String, Pattern> stringPatternCache = new GenericLastValuesCache<String, Pattern>() {
		/**
		 * 
		 */
		private static final long serialVersionUID = 7984225640937449707L;
		...
		@Override
		protected Pattern resolve(final String key) {
			String sPattern = Formatare.replace(key, "*", ".*");
	...
	static final public boolean matchString(final String p, final String d) {
		...
		final Pattern pattern = stringPatternCache.get(p);
