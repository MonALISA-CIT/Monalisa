...
public class monProcIO extends monProcReader {
	...
	private static final Pattern dockerVirtualInterface = Pattern.compile("^veth[a-z0-9]{7}$");

	private static Set<String> blackListSubInterfaces(List<String> allLines) {
		final Map<MacAddr, String> discoveredInterfaces = new HashMap<>();

		final Set<String> blackListedInterfaces = new HashSet<>();
		...
		for (String line : allLines) {
			try {
				...
				boolean bCount = !dockerVirtualInterface.matcher(iName).matches();
				...
				if (bCount) {
					...
					logger.log(Level.FINE, "[monProcIO] ignoring docker virtual interface '" + iName + "'");
