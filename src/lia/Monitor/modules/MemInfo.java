...
public class MemInfo extends AbstractSchJobMonitoring {
	...
	public Object doProcess() throws Exception {
		...
		if (isLinuxOS()) {
			try (BufferedReader br = new BufferedReader(new FileReader("/proc/meminfo"))) {
				...
				while ((sLine = br.readLine()) != null) {
					...
					if (s.equals("MemFree")) {
						...
						if (s.equals("MemTotal")) {
							total_mem = value;
						}
						else
							if (s.equals("SwapFree")) {
								swap_free = value;
							}
							else
								if (s.equals("SwapTotal")) {
									total_swap = value;
								}
								else
									if (s.equals("Buffers")) {
			...
			if (mem_available > 0) {
				final double delta = mem_available - mem_actualfree;

				if (delta > 0 && delta + mem_cached > 0) {
					mem_actualfree = mem_available;
					mem_cached += delta;
				}
			}
			...
			if (isSolarisOS()) {
				...
				try {
					...
					try (BufferedReader br = new BufferedReader(new StringReader(vmstat))) {
				...
				try {
					...
					try (BufferedReader br = new BufferedReader(new StringReader(swap))) {
