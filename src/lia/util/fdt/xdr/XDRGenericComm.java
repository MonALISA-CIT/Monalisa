...
public abstract class XDRGenericComm implements Runnable {
    ...
    private static final Logger logger = Logger.getLogger(XDRGenericComm.class.getName());
    ...
    public void run() {
        try {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                logger.log(Level.WARNING, "Session [" + System.currentTimeMillis() + " ] " + myName + " K: ["
                        + getKey() + "] cannot be initialized..closing", e);
            }
            notifyXDRCommClosed();
            close();
            return;
        ...
        try {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                logger.log(Level.WARNING, " [ " + System.currentTimeMillis() + " ] " + getMyName()
                        + " is broken. Closing it.. ");
            }
        ...
        if (logger.isLoggable(Level.INFO)) {
            logger.log(Level.INFO, " [ " + System.currentTimeMillis() + " ] " + myName + " K: [" + getKey()
                    + "] exits now .... \n\n");
        }
        close();
    }
    ...
    public XDRMessage read() throws IOException {
        try {
            ...
            logger.log(Level.INFO, " [ " + System.currentTimeMillis() + " ] " + getMyName()
                    + ": Connection closed by remote host");
            throw new IOException(" [ " + System.currentTimeMillis() + " ] " + getMyName()
                    + ": Connection closed by remote host");
        } catch (Throwable t) {
